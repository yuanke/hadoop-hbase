...
public class Counters implements Writable, Iterable<Counters.Group> {
  ...
  public class Group implements Writable, Iterable<Counter> {
    ...
    public String makeEscapedCompactString() {
      ...
      // First up, obtain the strings than need escaping. This will help us
      // determine the buffer length apriori.
      String escapedName = escape(getName());
      String escapedDispName = escape(getDisplayName());
      int i = 0;
    ...
    public synchronized Counter getCounterForName(String name) {
      ...
      if (result == null) {
        ...
        if (numCounters > MAX_COUNTER_LIMIT) {
          throw new CountersExceededException(
              "Exceed the max number of counters per job: " + "Num = "
                  + numCounters + " Limit: " + MAX_COUNTER_LIMIT);
        }
        if (name.length() > COUNTER_NAME_LIMIT) {
          throw new CountersExceededException(
              "Exceed the max string size of a counter name: " + "CounterName = "
                  + name + " Limit: " + COUNTER_NAME_LIMIT);
        }
  ...
  public synchronized Group getGroup(String groupName) {
    ...
    if (result == null) {
      if (groupName.length() > GROUP_NAME_LIMIT) {
        throw new CountersExceededException(
            "Exceed the max string size of a group name: " + "GroupName = "
                + groupName + " Limit: " + GROUP_NAME_LIMIT);
      }
