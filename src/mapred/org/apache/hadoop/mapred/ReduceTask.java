...
import org.apache.hadoop.mapred.MapOutputWatcher.ShuffleRefusedException;
...
class ReduceTask extends Task {
  ...
  class ReduceCopier<K, V> implements MRConstants {
    ...
    class ShuffleClientMetrics implements Updater {
      ...
      private int numRefusedFetches = 0;
      ...
      public synchronized void refusedFetch() {
        ++numRefusedFetches;
      }
      ...
      public void doUpdates(MetricsContext unused) {
        synchronized (this) {
          ...
          shuffleMetrics.incrMetric("shuffle_success_fetches", 
                                    numSuccessFetches);
          shuffleMetrics.incrMetric("shuffle_refused_fetches", 
                                    numRefusedFetches);
          if (numCopiers != 0) {
    ...
    private class CopyResult {
      ...
      //a flag signifying whether a copy result is invalid because the shuffle was refused
    ...
    class ShuffleRamManager implements RamManager {
      ...
      public ShuffleRamManager(Configuration conf) throws IOException {
        ...
        maxSize = (int)(conf.getInt("mapred.job.reduce.total.mem.bytes",
            (int)Math.min(Runtime.getRuntime().maxMemory() * maxInMemCopyUse, Integer.MAX_VALUE)));
    ...
    private class MapOutputCopier extends Thread {
        ...
        while (true) {        
          try {
            ...
            try {
              ...
              if (e instanceof ShuffleRefusedException) {
                LOG.warn(reduceTask.getTaskID() + " copy was refused: " +
                    loc.getTaskAttemptId() + " from " + loc.getHost());
                LOG.warn(StringUtils.stringifyException(e));
                shuffleClientMetrics.refusedFetch();
                
                // Set refused flag
                size = CopyResult.REFUSED;
              } else {
      ...
      throws IOException, InterruptedException {
        ...
        //Here check the http header to decide to read data or stop
        int servletStatus = Integer.parseInt(connection.getHeaderField(MAP_OUTPUT_SERVLET_STATUS));
        if (servletStatus == -1) {
          // Close this stream
          IOUtils.cleanup(LOG, input);
          throw new ShuffleRefusedException("This shuffle was refused by " + mapOutputLoc.getHost()
              + " for too many worker threads were occupied by our job.");
        }
          ...
          while (numInFlight > 0 && mergeThrowable == null) {
            ...
            if (cr.getSuccess()) {  // a successful copy
              ...
              LOG.info(reduceTask.getTaskID() + 
                       ...
                       cr.getHost());
            } else if (cr.isRefused()) {
              // This shuffle was refused by TaskTracker, for too many reduces are shuffling one map 
              // output or too many reduces of our job are connected to one TaskTracker. Anyway, 
              // in this case, the shuffle just need to retry later.
              retryFetches.add(cr.getLocation());
              currentTime = System.currentTimeMillis();
              penaltyBox.put(cr.getHost(), currentTime + BACKOFF_INIT);
            } else {
              retryFetches.add(cr.getLocation());
