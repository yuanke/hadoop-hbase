...
import org.apache.hadoop.mapred.MapOutputWatcher;
...
public class TaskTracker 
  ...
  /**
   * Watch the map output servlet, protect threads from occupied by one big job.
   */
  private MapOutputWatcher mapOutputWatcher;
  ...
  private class ShuffleServerMetrics implements Updater {
    ...
    synchronized void refusedOutput() {
      ++refusedOutputs;
    }
    public void doUpdates(MetricsContext unused) {
      synchronized (this) {
        ...
        shuffleMetricsRecord.incrMetric("shuffle_success_outputs", 
                                        successOutputs);
        shuffleMetricsRecord.incrMetric("shuffle_refused_outputs", 
                                        refusedOutputs);
        outputBytes = 0;
        failedOutputs = 0;
        successOutputs = 0;
   ...
   * @param tid the TaskAttampID of the task for which pidFile's path is needed
   * @param conf Configuration for local dir allocator
   * @param isCleanup true if the task is cleanup attempt
   * @return pidFile's Path
   * @author liyin.liangly
   */
  public static Path getPidFilePath(TaskAttemptID tid,
                                    JobConf conf, 
                                    boolean isCleanup) {
    ...
    try {
      ...
      pidFileName = lDirAlloc.getLocalPathToRead(
          TaskTracker.getPidFile(tid.getJobID().toString(), tid.toString(), isCleanup), conf);
    } catch (IOException i) {
  ...
  synchronized void initialize() throws IOException {
    ...
    //check local disk and remove bad ones.
    removeBadLocalDirs(this.fConf.getLocalDirs());
  ...
  /**
   * Create a new {@link TaskTracker}, for test only.
  ...
  public TaskTracker(JobConf conf) throws IOException {
    ...
    this.mapOutputWatcher = new MapOutputWatcher(conf);
    ...
    server.setAttribute("mapOutputWatcher", mapOutputWatcher);
  ...
  private HeartbeatResponse transmitHeartBeat(long now) throws IOException {
    ...
    // Don't ask for new task when report a failed Job setup/cleanup task 
    // to avoid accepting the same setup/cleanup task's attempt. added by liangly 
    if (askForNewTask && hasFailedJobSetupCleanupTask()) {
      askForNewTask = false;
    }
  ...
  /**
   * Whether this TT has job setup/cleanup task who's running state is failed 
   * to report to JT. 
   * @return has or not
   * @author liangly
   */
  private boolean hasFailedJobSetupCleanupTask() {
    for (TaskStatus taskStatus : status.getTaskReports()) {
      ...
      if ((taskStatus.getRunState() == TaskStatus.State.FAILED)
          && (task.isJobSetupTask() || task.isJobCleanupTask())) {
        return true;
      }
    }
    return false;
  ...
  long tryToGetOutputSize(TaskAttemptID taskId, JobConf conf) {
    ...
    try{
      ...
      if (!tip.getTask().isMapTask() || !tip.getTask().isMapOrReduce() ||
  ...
  /**
   * Check if the given local directories are writable. If found one or more
   * bad ones, remove from Conf's {mapred.local.dir}.
   * @param localDirs  where the new TaskTracker should keep its local files.
   * @throws DiskErrorException  if all local directories are not writable
   * @author liangly
   */
  private void removeBadLocalDirs(String[] localDirs) throws DiskErrorException {
    boolean writable = false;
    List<String> badDirs = new ArrayList<String>();
    ...
    if (localDirs != null) {
      for (int i = 0; i < localDirs.length; i++) {
        try {
          ...
          badDirs.add(localDirs[i]);
        }
      }
    }

    if (!badDirs.isEmpty()) {
      // remove bad local directories from {mapred.local.dir}
      String[] dirs = badDirs.toArray(new String[0]);
      removeBadMapredLocalDirs(this.originalConf, dirs);
      fConf.set("mapred.local.dir", originalConf.get("mapred.local.dir"));
    }

    if (!writable)
      throw new DiskErrorException("all local directories are not wirtable");
  }
  ...
  public static class MapOutputServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, 
      ...
      MapOutputWatcher mapOutputWatcher = 
        (MapOutputWatcher) context.getAttribute("mapOutputWatcher");

      String localDir = ""; // local dir of map output file.
      try {
       ...
       IndexRecord info = 
        ...
        // extract local directory form map output file
        String filename = mapOutputFileName.toString();
        String postfix = TaskTracker.getIntermediateOutputDir(jobId, mapId) + "/file.out";
        localDir = filename.substring(0, filename.length() - postfix.length());
        if (mapOutputWatcher.canShuffleAndInr(localDir, jobId, rawPartLength)) {
          //set the custom "Map-Output-Servlet-Status" http header to zero.
          //The client will continue this shuffle.
          response.setHeader(MAP_OUTPUT_SERVLET_STATUS, "0");
        } else {
          //set the custom "Map-Output-Servlet-Status" http header to -1. Too many
          // worker threads are occupied by this job. So the client will stop shuffle
          // and try again later.
          response.setHeader(MAP_OUTPUT_SERVLET_STATUS, "-1");
          shuffleMetrics.refusedOutput();
          LOG.info("Refuse shuffle for reduce: " + reduce + " from map: " + mapId);
          return;
        }
        ...
        mapOutputWatcher.shuffleDecr(localDir, jobId);
  ...
  /**
   * delete the bad mapred.local.dir item from conf
   * 
   * @param conf Configuration
   * @param badDir array of bad directory to be removed from conf
   */
  private static void removeBadMapredLocalDirs(JobConf conf, String badDir[]) {
    String oldLocalDirs = conf.get("mapred.local.dir");
    List<String> dirs = (List<String>) StringUtils.getStringCollection(oldLocalDirs);

    for (int i = 0; i < badDir.length; i++) {
      if (dirs.contains(badDir[i])) {
        dirs.remove(badDir[i]);
      }
    }

    if (!dirs.isEmpty()) {
      conf.setStrings("mapred.local.dir", dirs.toArray(new String[0]));
    } else {
