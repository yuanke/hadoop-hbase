...
class JobInProgress {
  ...
  public JobInProgress(JobID jobid, JobTracker jobtracker, 
    ...
    copyToLocalFile(fs, jobFile, localJobFile);
    ...
    if (jarFile != null) {
      copyToLocalFile(fs, new Path(jarFile), localJarFile);
  ...
  // hack by DC: to avoid chmod operation when copy files to local
  private void copyToLocalFile(FileSystem srcFS, Path src, Path dst) 
      throws IOException{
    ...
    try {
      in = srcFS.open(src);
      // block size, replica number and Progressable are not applicable
      //  and set FsPermission to null on purpose.
      out = dstFS.create(dst, null, true, 
  ...
  public synchronized void initTasks() throws IOException, KillInterruptedException {
    ...
    if (numMapTasks == 0) {
      ...
      JobHistory.JobInfo.logFinished(profile.getJobID(), 
                                     ...
                                     getCounters(), getMapCounters(), getReduceCounters());
   ...
   *  Fail this job if counter exceeded.
   */
  public Counters getMapCounters() {
    ...
    synchronized(this) {
      try {
        ...
        addDiagnosticInfoToActiveTasks(StringUtils.stringifyException(ce));
        isFine = false;
      }
    }
    if (!isFine) {
      LOG.info("Fail job " + jobId + " for counters exceeded.");
  ...
  public Counters getCounters() {
    ...
    try {
      ...
      // hence read-only access is ok without any lock
      incrementTaskCounters(result, maps);
      incrementTaskCounters(result, reduces);
    } catch (CountersExceededException ce) {
      LOG.info("Counters Exceeded for Job: " + jobId, ce);
  ...
  /**
   * Add diagnostic information to all the running map/reduce attempts
   * @param diagInfo diagnostic information
   */
  private synchronized void addDiagnosticInfoToActiveTasks(String diagInfo) {
    for (int i = 0; i < maps.length; i++) {
      Set<TaskAttemptID> attempts = maps[i].getActiveTasks().keySet();
      for (TaskAttemptID attempt : attempts) {
        maps[i].addDiagnosticInfo(attempt, diagInfo);
      }
    }
    for (int i = 0; i < reduces.length; i++) {
      Set<TaskAttemptID> attempts = reduces[i].getActiveTasks().keySet();
      for (TaskAttemptID attempt : attempts) {
        reduces[i].addDiagnosticInfo(attempt, diagInfo);
      }
    }
  }
  ...
  public synchronized Task obtainNewMapTask(TaskTrackerStatus tts, 
    ...
    int target = findNewMapTask(tts, clusterSize, numUniqueHosts, 
    		maxCacheLevel, status.mapProgress());
    if (target == -1) {
      return null;
    }
  ...
  public synchronized Task obtainNewMapTask(TaskTrackerStatus tts, 
  	...
  	return obtainNewMapTask(tts, clusterSize, numUniqueHosts, 
  			NetworkTopology.DEFAULT_HOST_LEVEL+1);
  }

  public boolean getTaskCleanupTasks(boolean isMapSlot)
      throws IOException {
    if (!tasksInited.get()) {
      return false;
    }
  ...
  public Task launchTaskCleanupTask(String taskTracker, boolean isMapSlot)
      throws IOException {
    synchronized (this) {
  ...
  public List<TaskInProgress> getJobCleanupTasks() throws IOException {
    if(!tasksInited.get()) {
      return null;
    }
    ...
    synchronized(this) {
      ...
      return cleanupTaskList;
    }
  }
    
  public Task launchJobCleanupSetupTask(TaskTrackerStatus tts, int clusterSize,
      int numUniqueHosts, TaskInProgress tipNeedLaunch)
      throws IOException {
    synchronized (this) {
      if (tipNeedLaunch.isJobCleanupTask()) {
        if (!canLaunchJobCleanupTask()) {
          return null;
        }
      }
      ...
      List<TaskInProgress> taskList = new ArrayList<TaskInProgress>();
      taskList.add(tipNeedLaunch);
      TaskInProgress tip = findTaskFromList(taskList,
  ...
  public List<TaskInProgress> getJobSetupTasks() throws IOException {
    if (!tasksInited.get()) {
      return null;
    }
                                         ...
                                         JobTrackerInstrumentation metrics) 
  {
    ...
    if (tip.isJobSetupTask()) {
      ...
      // move the job to running state if the job is in prep state.(HADOOP-5636)
  ...
  private void jobComplete(JobTrackerInstrumentation metrics) {
    ...
    if (this.status.getRunState() == JobStatus.RUNNING ) {
      ...
      JobHistory.JobInfo.logFinished(this.status.getJobID(), finishTime, 
                                     ...
                                     failedReduceTasks, getCounters(), getMapCounters(), getReduceCounters());
   ...
   * {@link JobTracker#failedJobs(JobInProgress)} for more details.
   */
  ...
  /**
   * Get the level of locality that a given task would have if launched on a
   * particular TaskTracker. Returns 0 if the task has data on that machine, 1
   * if it has data on the same rack, etc (depending on number of levels in the
   * network hierarchy).
   */
