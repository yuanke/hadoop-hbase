...
import org.apache.hadoop.security.UserPasswordInformation;
...
public class JobTracker implements MRConstants, InterTrackerProtocol,
  ...
  static Map<String,TimePattern> queueSubmitTimesMap; // added by luoli
  static Map<String,Boolean> queueUseOtherSlotsMap; // added by luoli
  static int DEFAULT_RECLAIM_TIME_LIMIT = 15000; // added by luoli
  static long JOB_SLOTS_UPDATE_INTERVAL = 1000; // added by luoli
  // Approximate number of heartbeats that could arrive JobTracker
  // in a second
  private static int NUM_HEARTBEATS_IN_SECOND = 100;
  ...
  private List<TaskInProgress> jobCleanupSetupMapTasks = new LinkedList<TaskInProgress>();
  private List<TaskInProgress> jobCleanupSetupReduceTasks = new LinkedList<TaskInProgress>();
  private List<JobInProgress> taskCleanupMapTasks = new LinkedList<JobInProgress>();
  private List<JobInProgress> taskCleanupReduceTasks = new LinkedList<JobInProgress>();
  ...
  private class SetupCleanupTasks implements Runnable {

    // returns cleanup tasks first, then setup tasks.
    private synchronized void updateSetupAndCleanupTasks() throws IOException {      
                 
      List<TaskInProgress> tips = null;
      synchronized (JobTracker.this) {
        synchronized (jobs) {
          
          jobCleanupSetupMapTasks.clear();
          jobCleanupSetupReduceTasks.clear();
          taskCleanupMapTasks.clear();
          taskCleanupReduceTasks.clear();
          
          for (Iterator<JobInProgress> it = jobs.values().iterator(); it
              .hasNext();) {
            JobInProgress job = it.next();
            tips = job.getJobCleanupTasks();
            if (tips != null) {
              jobCleanupSetupMapTasks.add(tips.get(0));
              jobCleanupSetupReduceTasks.add(tips.get(1));
            }

            if (job.getTaskCleanupTasks(true)) {
              taskCleanupMapTasks.add(job);
            }

            if (job.getTaskCleanupTasks(false)) {
              taskCleanupReduceTasks.add(job);
            }

            tips = job.getJobSetupTasks();
            if (tips != null) {
              jobCleanupSetupMapTasks.add(tips.get(0));
              jobCleanupSetupReduceTasks.add(tips.get(1));
            }
    ...
    public void run() {
      while (true) {
        try {
          updateSetupAndCleanupTasks();
          Thread.sleep(1000);
        } catch (InterruptedException t) {
          break;
        } catch (Exception e) {
          LOG.error("SetupCleanupTasks got exception: "
              + StringUtils.stringifyException(e));
        }
      }
    }    
  ...
  class RecoveryManager {
    ...
    public void recover() throws IOException {
      ...
      for (Iterator<JobID> it = jobsToRecover.iterator(); it.hasNext(); ) {
    	JobID id = it.next();
    	JobInProgress job = null;
        // 1. Create the job object
        try {
          job = new JobInProgress(id, JobTracker.this, conf);
        } catch (RuntimeException e) {
          if(e.getCause() instanceof SAXParseException) {
            //DCCOMMENT add by zyj, remove this dir, or it will cause SAXParseException when jobtracker try to recover jobs
            fs.delete(new Path(systemDir, id.toString()) ,true);
            LOG.info("In recover, delete job file: " + new Path(systemDir, id.toString()));             
            it.remove();
            continue;
          }
        }
  ...
  // (trackerID->HashSet of taskids running at that tracker)
  ...
  SetupCleanupTasks setupCleanupTasks = new SetupCleanupTasks();
  Thread setupCleanupTasksThread = new Thread(setupCleanupTasks, "SetupCleanupTasks");
  ...
  private UserPasswordInformation userPasswordInformation;
  
  //TO BE USED BY TEST CLASSES ONLY
  //ONLY BUILD THE STATE WHICH IS REQUIRED BY TESTS
  JobTracker() {
    MAX_COMPLETE_USER_JOBS_IN_MEMORY = 100;
    hostsReader = null;
    infoServer = null;
    queueManager = null;
    taskScheduler = null;
  ...
  JobTracker(JobConf conf) throws IOException, InterruptedException {
    ...
    queueSubmitTimesMap = new HashMap<String, TimePattern>(); // added by luoli
    queueUseOtherSlotsMap = new HashMap<String, Boolean>(); // added by luoli
    DEFAULT_RECLAIM_TIME_LIMIT = conf.getInt("default.reclaim.time.interval", 15000);
    JOB_SLOTS_UPDATE_INTERVAL = conf.getLong("job.slots.update.interval", 1000);
  ...
  public void offerService() throws InterruptedException, IOException {
    ...
    this.setupCleanupTasksThread.start();
  ...
  void close() throws IOException {
    ...
    if (this.setupCleanupTasksThread != null && this.setupCleanupTasksThread.isAlive()) {
      LOG.info("Stopping setupCleanupTasksThread thread");
      this.setupCleanupTasksThread.interrupt();
      try {
        this.setupCleanupTasksThread.join();
      } catch (InterruptedException ex) {
        ex.printStackTrace();
      }
    }
  ...
  void removeTaskEntry(TaskAttemptID taskid) {
    ...
    // tracker --> taskid
    if (tracker != null) {
      Set<TaskAttemptIDWithTip> trackerSet = trackerToTaskMap.get(tracker);
      if (trackerSet != null) {
        // TaskAttemptIDWithTip's comparison function does't care tip, so object with
        // the same taskid in trackerSet will be removed.
        trackerSet.remove(new TaskAttemptIDWithTip(taskid, null));
      }
    }
  ...
  public synchronized HeartbeatResponse heartbeat(TaskTrackerStatus status, 
    ...
    if (acceptNewTasks) {
      ...
      if (taskTrackerStatus == null) {
        ...
        long assigStartTime = System.currentTimeMillis();
        if (tasks == null ) {
          tasks = taskScheduler.assignTasks(taskTrackerStatus);
        }
        long elapse = System.currentTimeMillis() - assigStartTime;
        if (tasks != null) {          
          for (Task task : tasks) {
        ...
        if (assignedMaps > 0) {          
          myInstrumentation.assignedMap.inc(assignedMaps, elapse);
        }
        if (assignedReduces > 0) {
          myInstrumentation.assignedReduce.inc(assignedReduces, elapse);
        }
      }
    }
    ...
    myInstrumentation.heartBeat.inc(System.currentTimeMillis() - heartBeatStartTime);
  ...
  */
  ...
  private List<Task> launchJobCleanupSetupTask (TaskTrackerStatus taskTracker, 
                                                int numTaskTrackers, 
                                                int numUniqueHosts,
                                                boolean isMap) throws IOException  {
    Iterator<TaskInProgress> tipIter;
    if (isMap) {
      tipIter = jobCleanupSetupMapTasks.iterator();
    } else {
      tipIter = jobCleanupSetupReduceTasks.iterator();
    }
    Task t = null;
    JobInProgress job = null;
    TaskInProgress tip = null;
    while (tipIter.hasNext()) {
      tip = tipIter.next();
      job = tip.getJob();
      t = job.launchJobCleanupSetupTask(taskTracker, numTaskTrackers,
          numUniqueHosts, tip);
      if (t != null) {
        tipIter.remove();
  ...
  private List<Task> launchTaskCleanupTask (String taskTrackerName,
                                             boolean isMap) throws IOException {
    Iterator<JobInProgress> jobIter;
    if (isMap) {
      jobIter = taskCleanupMapTasks.iterator();
    } else {
      jobIter = taskCleanupReduceTasks.iterator();
    }
    Task t = null;
    JobInProgress job = null;
    while (jobIter.hasNext()) {
      job = jobIter.next();
      if (!job.shouldRunOnTaskTracker(taskTrackerName)) {
        continue;
      }
      t = job.launchTaskCleanupTask(taskTrackerName, isMap);
      if (t != null) {
        return Collections.singletonList(t);
      } else {
        jobIter.remove();
  ...
  private synchronized List<Task> getSetupAndCleanupTasks(
    ...
    boolean hasMapSlot = taskTracker.countMapTasks() < taskTracker
        .getMaxMapTasks();
    boolean hasReduceSlot = taskTracker.countReduceTasks() < taskTracker
        .getMaxReduceTasks();
    ...
    synchronized (jobs) {      
      if (hasMapSlot) {        
        tasks = launchJobCleanupSetupTask(taskTracker, numTaskTrackers, numUniqueHosts, true);
        if (tasks != null) {
          return tasks;
        }
        
        tasks = launchTaskCleanupTask(taskTrackerName, true);
        if (tasks != null) {
          return tasks;
        }
      }
      ...
      if (hasReduceSlot) {        
        tasks = launchJobCleanupSetupTask(taskTracker, numTaskTrackers, numUniqueHosts, false);
        if (tasks != null) {
          return tasks;
        }
        
        tasks = launchTaskCleanupTask(taskTrackerName, false);
        if (tasks != null) {
          return tasks;
        }
      }
  ...
  public JobID getNewJobId() throws IOException {
    return new JobID(getTrackerIdentifier(), nextJobId.getAndIncrement());
  }
  ...
  public JobStatus submitJob(JobID jobId) throws IOException {
    ...
    synchronized (this) {
      ...
      checkSubmitPattern(job, QueueManager.QueueOperation.SUBMIT_JOB); // added by luoli
  ...
  private void checkAccess(JobInProgress job, 
    ...
    if (!userPasswordInformation.checkUserAndPassword(ugi.getUserName(), ugi.getPassword(), ugi.getGroupNames())) {
    	throw new AccessControlException("User " + ugi.getUserName() + " login failed.");
    }
    
    // get the queue
    String queue = job.getProfile().getQueueName();
    if (!queueManager.hasAccess(queue, job, oper, ugi)) {
      //DCCOMMENT add by zyj, remove this dir, or it will cause nullPointerException when jobtracker try to recover jobs
      fs.delete(new Path(systemDir, job.getStatus().getJobID().toString()) ,true);
      LOG.info("In checkAccess, delete dir " + new Path(systemDir, job.getStatus().getJobID().toString()));
  ...
  public synchronized void killJob(JobID jobid) throws IOException {
    ...
    // log job killer
    UserGroupInformation ugi = UserGroupInformation.getCurrentUGI();
    LOG.info(ugi.toString() + " wants to kill job " + jobid);
    killJob(job);
  }
  ...
  /**
   * Init a job and inform the listeners. Other components in the framework 
   * should use this to init a job.
   */
  ...
  public JobProfile getJobProfile(JobID jobid) {
    synchronized (this) {
      JobInProgress job = jobs.get(jobid);
      if (job != null) {
        // Safe to cal JobInProgress.getProfile while holding the lock
   ...
   * @return <code>true</code> if the job has been initialzed
  ...
  private static final org.apache.hadoop.mapred.Counters EMPTY_COUNTERS
  ...
  public TaskCompletionEvent[] getTaskCompletionEvents(
    ...
    if (null != job) {
      return isJobInited(job) ? job.getTaskCompletionEvents(fromEventId, maxEvents)
  ...
  void updateTaskStatuses(TaskTrackerStatus status) {
    ...
    for (TaskStatus report : status.getTaskReports()) {
      ...
      if (tip != null || hasRestarted()) {
        //JobInProgress job = getJob(taskId.getJobID()); // from patch-5247
        if (tip == null) {
          tip = job.getTaskInProgress(taskId.getTaskID());
          job.addRunningTaskToTIP(tip, taskId, status, false);
        }
        //expireLaunchingTasks.removeTask(taskId); // from patch-5247
  ...
  private static void reloadSubmitPattern(){
    String prefix = "mapred.queue.";
    String suffix = ".submittime.pattern";
    JobConf jc = new JobConf();
    RETIRE_JOB_INTERVAL = jc.getLong("mapred.jobtracker.retirejob.interval", 0);
    String allQueuesStr = jc.get("mapred.queue.names", "cug-default");
    String[] queues = allQueuesStr.split("\\s*,\\s*");
    synchronized (queueSubmitTimesMap) {
      queueSubmitTimesMap.clear();
      for (String queue : queues) {
        String queueSubmitPatternConf = prefix + queue + suffix;
        String queueSubmitPattern = jc.get(queueSubmitPatternConf, "*;*;*;*;*");
        queueSubmitTimesMap.put(queue, new TimePattern(queueSubmitPattern));
      }
    }
    
    suffix = ".use.otherslots";
    synchronized (queueUseOtherSlotsMap) {
      queueUseOtherSlotsMap.clear();
      for (String queue : queues) {
        String queueSubmitPatternConf = prefix + queue + suffix;
        boolean isQueueUseOtherSlots = jc.getBoolean(queueSubmitPatternConf, true);
        queueUseOtherSlotsMap.put(queue, isQueueUseOtherSlots);
      }
    }
    DEFAULT_RECLAIM_TIME_LIMIT = jc.getInt("default.reclaim.time.interval", 15000);
    RETIRE_JOB_CHECK_INTERVAL = jc.getLong("mapred.jobtracker.retirejob.check", 60 * 1000);
    JOB_SLOTS_UPDATE_INTERVAL = jc.getLong("job.slots.update.interval", 1000);
    NUM_HEARTBEATS_IN_SECOND = jc.getInt("mapred.heartbeats.in.second", 100);
  }
  
  private void checkSubmitPattern(JobInProgress job, QueueManager.QueueOperation oper) throws IOException {
	  String queue = job.getProfile().getQueueName();
	  synchronized (queueSubmitTimesMap) {
		  TimePattern timePattern = queueSubmitTimesMap.get(queue);
		  if(timePattern != null && !timePattern.isCanSubmitNow()){
			  throw new AccessControlException("Group of " 
					  + queue
					  + " cannot perform "
					  + "operation " + oper + " AT PRESENT.");
		  }
	  }
  }
  
  public static boolean isQueueUseOtherSlots(String queueName) {
	  synchronized (queueUseOtherSlotsMap) {
		  Boolean isUse = queueUseOtherSlotsMap.get(queueName);
		  return (null == isUse) ? true : isUse;
	  }
  }

  public static int getReclaimInterval() {
	  return DEFAULT_RECLAIM_TIME_LIMIT;
  }
  
  public static long getJobSlotsUpdateInterval() {
	  return JOB_SLOTS_UPDATE_INTERVAL;
  }

  public boolean isClusterBusy(String taskType) {
	  if(taskType.equalsIgnoreCase("map"))
		  return totalMaps >= totalMapTaskCapacity;
	  else
		  return totalReduces >= totalReduceTaskCapacity;
  }
