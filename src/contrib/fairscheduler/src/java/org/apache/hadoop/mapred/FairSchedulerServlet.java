...
public class FairSchedulerServlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    // disable admin view for bug 51268  ----modified by liangly
    boolean adminView = false;

    // Print out the normal response
	String adv = advancedView ? "&" : "?";
    ...
    // adjust refresh interval from 15 second to 1 minute ----modified by liangly 
    out.printf("<META http-equiv=\"refresh\" content=\"60;URL=/scheduler%s%s\">",
            advancedView ? "?advanced=true" : "",adminView ? adv + "admin=true" : "");
    ...
    if(adminView){
    	...
    	showJobsWithLimit(out, advancedView);
    }
    out.print("</body></html>\n");
    out.close();
  }
  ...
  private void showJobs(PrintWriter out, boolean advancedView) {
    ...
    for (JobInProgress job: runningJobs) {
      ...
      out.printf("<td>%s</td>\n", profile.getUser());
      out.printf("<td>%s</td>\n", profile.getJobName());
      // Job Pool should be read-only for bug 51268 --modified by liangly
      out.printf("<td>%s</td>\n", scheduler.getPoolManager().getPoolName(job));
      // Job Type should be read-only fro bug 51268 --modified by liangly
      out.printf("<td>%s<td>\n", FairScheduler.isProductJob(job) ? "production":"experimental");
      // Job Priority should be read-only for bug 51268 --modified by liangly
  ...
  private void showAdminForm(PrintWriter out, boolean advancedView) {
    ...
    String advParam = advancedView ? "?advanced=true" : "";
  ...
  /**
   * Print a view of running jobs with no right to change
   * the jobs' priority and pool to the given output writer
   * 
   * Added by wukong
   */
  
  private void showJobsWithLimit(PrintWriter out, boolean advancedView) {
	    out.print("<h2>Running Jobs</h2>\n");
	    out.print("<table border=\"2\" cellpadding=\"5\" cellspacing=\"2\">\n");
	    int colsPerTaskType = advancedView ? 5 : 3;
	    out.printf("<tr><th rowspan=2>Submitted</th>" + 
	    ...
	    for (JobInProgress job: runningJobs) {
	      ...
	      // show the job pool --added by liangly
	      out.printf("<td>%s</td>\n", scheduler.getPoolManager().getPoolName(job));
	      // Job type should be read-only for bug 51268  --modified by liangly
	      out.printf("<td>%s</td>\n", FairScheduler.isProductJob(job) ? "production":"experimental");
	   // Job Priority should be read-only for bug 51268 --modified by liangly
	      out.printf("<td>%s</td>\n", job.getPriority().toString());
