...
public class YuntiFsShell extends FsShell {

  protected FileSystem fs;
  private static String cmdOption;
  private static boolean archiveFileSystem;
  private final String fileSystemPropertyName  = "fs.hdfs.impl";
  private final String fileSystemPropertyValue = "org.apache.hadoop.hdfs.YuntiDistributedFileSystem";
  private Trash trash;
  ...
  public YuntiFsShell(Configuration conf) {
    super(conf);
    fs = null;
    trash = null;
  }
  ...
  private void dirDiffCompare(FileStatus[] online, FileStatus[] archive,
      String printInfo) {
    int i = 0, j = 0;
    String onlinePath, archivePath;
    while (i < online.length && j < archive.length) {
      onlinePath = online[i].getPath().toUri().getRawPath();
      archivePath = archive[j].getPath().toUri().getRawPath();
      int cmpR = onlinePath.compareTo(archivePath);
      if (cmpR == 0) {
        System.out.println("Common " + printInfo + ": " + onlinePath + " and "
            + archivePath);
        i++;
        j++;
      } else if (cmpR < 0) {
        System.out.println("Only in online: " + onlinePath);
        i++;
      } else {
        System.out.println("Only in archive: " + archivePath);
        j++;
      }
    }
    while (i < online.length) {
      System.out.println("Only in online: "
          + online[i].getPath().toUri().getRawPath());
      i++;
    }
    while (j < archive.length) {
      System.out.println("Only in archive: "
          + archive[j].getPath().toUri().getRawPath());
      j++;
    }
  }

  private int dirDiffChoose(FileStatus[] srcsOnline, FileStatus[] srcsArchive,
      boolean srcsOnline0, boolean srcsArchive0, boolean toplevel, String srcf,
      FileSystem srcFs) throws IOException {
    if (srcsOnline0 && srcsArchive0) {
      if (toplevel) {
        System.err.println("--- online --- Cannot access " + srcf
            + ": No such file or directory.");
        System.err.println("--- archive --- Cannot access " + srcf
            + ": No such file or directory.");
        return -1;
      } else {
        return 1;
      }
    } else if (srcsOnline0 && !srcsArchive0) {
      for (int i = 0; i < srcsArchive.length; i++) {
        System.out.println("Only in archive: "
            + srcsArchive[i].getPath().toUri().getRawPath());
      }
      return 0;
    } else if (!srcsOnline0 && srcsArchive0) {
      for (int i = 0; i < srcsOnline.length; i++) {
        System.out.println("Only in online: "
            + srcsOnline[i].getPath().toUri().getRawPath());
      }
      return 0;
    } else {
      if (toplevel) {
        if (srcsOnline.length == 1
            && srcsArchive.length == 1
            && srcsOnline[0].getPath().toUri().getRawPath()
                .equals(srcsArchive[0].getPath().toUri().getRawPath())) {
          return dirDiff(srcsOnline[0].getPath(), srcsArchive[0].getPath(),
              srcFs);
        } else {
          dirDiffCompare(srcsOnline, srcsArchive, "");
          return 0;
        }
      } else {
        dirDiffCompare(srcsOnline, srcsArchive, "subdirectories");
        return 0;
      }
    }
  }

  private int dirDiff(Path srcOnline, Path srcArchive, FileSystem srcFs)
      throws IOException {
    cmdOption = "-online";
    FileStatus[] itemsOnline;
    try {
      itemsOnline = shellListStatus("ls", srcFs, srcOnline);
    } catch (Exception e) {
      itemsOnline = null;
    }
    cmdOption = "-archive";
    FileStatus[] itemsArchive;
    try {
      itemsArchive = shellListStatus("ls", srcFs, srcArchive);
    } catch (Exception e) {
      itemsArchive = null;
    }
    boolean itemsOnline0 = itemsOnline == null;
    boolean itemsArchive0 = itemsArchive == null;
    return dirDiffChoose(itemsOnline, itemsArchive, itemsOnline0,
        itemsArchive0, false, null, null);
  }

  private int dirDiff(String srcf) throws IOException {
    Path srcPath = new Path(srcf);
    FileSystem srcFs = srcPath.getFileSystem(this.getConf());
    cmdOption = "-online";
    FileStatus[] srcsOnline;
    try {
      srcsOnline = srcFs.globStatus(srcPath);
    } catch (Exception e) {
      srcsOnline = null;
    }
    cmdOption = "-archive";
    FileStatus[] srcsArchive;
    try {    
      srcsArchive = srcFs.globStatus(srcPath);
    } catch (Exception e) {
      srcsArchive = null;
    }
    boolean srcsOnline0 = srcsOnline == null || srcsOnline.length == 0;
    boolean srcsArchive0 = srcsArchive == null || srcsArchive.length == 0;
    return dirDiffChoose(srcsOnline, srcsArchive, srcsOnline0, srcsArchive0,
        true, srcf, srcFs);
  }
  /**
  ...
  private void printHelp(String cmd) {
    ...
    */
    ...
    printHelpWithOption(cmd);
  }
  private static void printHelpWithOption(String cmd) {
    if(archiveFileSystem) {
      String header = "support Archive Cluster:";
      String ls = "-ls  [-online/-archive/-all/-diff] <path>";
      String cat= "-cat [-online/-archive/-all] <path>";
      String get= "-get [-ignoreCrc] [-crc] [-online/-archive/-all] <src> <localdst>";
      if("ls".equals(cmd)) {
  ...
  private void setCmdOption(String cmd, String argv[], int pos) {
    cmdOption = null;
    if(archiveFileSystem) {
      if("-get".equals(cmd)) {
        CommandFormat cf = new CommandFormat("copyToLocal", 2, 2, "crc",
            "ignoreCrc", "online", "archive", "all");
        try {
          List<String> parameters = cf.parse(argv, pos);
        } catch (IllegalArgumentException iae) {
          System.err.println("Usage: java FsShell " + GET_SHORT_USAGE);
        ...
        if (cf.getOpt("online")) {
          cmdOption = "-online";
        } else if (cf.getOpt("archive")) {
          cmdOption = "-archive";
        } else if (cf.getOpt("all")) {
          cmdOption = "-all";
        }
      }
      else if("-ls".equals(cmd)) {
        cmdOption = argv[pos];
        if(!getOpt("-online", "-archive", "-all", "-diff")) {
          cmdOption = null;
        }
      }
      else if("-cat".equals(cmd)) {
        cmdOption = argv[pos];
        if(!getOpt("-online", "-archive", "-all")) {
          cmdOption = null;
        }
      }
    }
  }
  public static String getCmdOption() {
    return cmdOption;
  }
  public static boolean getArchiveFileSystem() {
    return archiveFileSystem;
  }

  private boolean getOpt(String... possibleOpt) {
    if(cmdOption != null)
    {
      for (String opt : possibleOpt) {
        if (cmdOption.equals(opt)) {
          return true;
        }
      }
    }
  ...
  private void catWithOption(String src, boolean verifyChecksum)
      throws IOException {
    if (cmdOption.equals("-all")) {
      System.out.println("\t------ online ------");
      cmdOption = "-online";
      try {
        cat(src, true);
      } catch (Exception e) {
        System.err.println("cat : " + e.getLocalizedMessage());
      }
  ...
  private int lsWithOption(String srcf, boolean recursive) throws IOException {
    if (cmdOption.equals("-all")) {
      System.out.println("\t------ online ------");
      cmdOption = "-online";
      try {
        ...
        System.err.println("ls : " + e.getLocalizedMessage());
      }
      System.out.println("\t------ archive ------");
      cmdOption = "-archive";
      try {
        return ls(srcf, false);
      } catch (Exception e) {
        System.err.println("ls : " + e.getLocalizedMessage());
        return -1;
      }
    } else if (cmdOption.equals("-diff")) {
      return dirDiff(srcf);
    } else {
      return ls(srcf, false);
    }
  ...
  private int doall(String cmd, String argv[], int startindex) {
    ...
    for (; i < argv.length; i++) {
      try {        
        ...
        //
        if ("-cat".equals(cmd)) {
          cmdOption = argv[startindex];
          if (archiveFileSystem && getOpt("-online", "-archive", "-all")) {
            if (i == startindex) {
              i++;
            }
            catWithOption(argv[i], true);
          } else {
            cmdOption = null;
            cat(argv[i], true);
          }
          ...
          cmdOption = argv[startindex];
          if (archiveFileSystem && getOpt("-online", "-archive", "-all", "-diff")) {
            if (i == startindex) {
              i++;
            }
            exitCode = lsWithOption(argv[i], false);
          } else {
            cmdOption = null;
  ...
  void copyToLocalWithOption(String[] argv, int pos) throws IOException {
    if (cmdOption.equals("-all")) {
      cmdOption = "-online";
      try {
        copyToLocal(argv, pos);
      } catch (Exception re) {
  ...
  public int run(String argv[]) throws Exception {
    archiveFileSystem = getConf().get(fileSystemPropertyName).equals(fileSystemPropertyValue);
    if (argv.length < 1) {
      printUsage(""); 
      return -1;
    }
  ...
  /*
   * Require confirm for remove and move operation
   * Add by weichao
   */
  private static boolean confirmOperation(String argv[]) {
    boolean confirm = true;
    if (argv.length >= 1) {
      String cmd = argv[0];
      if (cmd.equals("-rm") || cmd.equals("-rmr") || cmd.equals("-mv") || cmd.equals("-expunge") || 
          cmd.equals("-chown") || cmd.equals("-chgrp")) {
        StringBuffer operation = new StringBuffer();
        for (int i = 0; i < argv.length - 1; i++) 
          operation.append(argv[i]+" ");
        operation.append(argv[argv.length-1]);
        System.out.print("\"" + operation.toString() + "\" (yes to confirm)?");
  ...
  public static void main(String argv[]) throws Exception {
    if (!confirmOperation(argv)) {
      System.exit(-1);
    }
    YuntiFsShell shell = new YuntiFsShell();
    int res;
    try {
