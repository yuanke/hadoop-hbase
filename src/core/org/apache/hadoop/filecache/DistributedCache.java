...
public class DistributedCache {
  ...
  private static TreeMap<Path, CacheDir> baseDirProperties =
  ...
  throws IOException {
    ...
    synchronized (cachedArchives) {
      ...
      if (lcacheStatus == null) {
        ...
        lcacheStatus = new CacheStatus(
          new Path(localPath.toString().replace(cachePath, "")), 
          localPath, subDir, uniqueString); 
    ...
    long allowedCacheSubdirs = conf.getLong(
        "local.cache.numberdirectories", DEFAULT_CACHE_SUBDIR_LIMIT);
    if (allowedSize < size || allowedCacheSubdirs < numberSubdirs) {
      // try some cache deletions
      LOG.info("Start to try some cache deletions.");
      deleteCache(conf);
      LOG.info("Cache deletions finished.");
    }
  ...
  private static void deleteCache(Configuration conf) throws IOException {
    ...
    for (CacheStatus lcacheStatus : deleteSet) {
      synchronized (lcacheStatus) {
        FileSystem.getLocal(conf).delete(lcacheStatus.getLocalizedUniqueDir(), true);
  ...
  private static class CacheStatus {
    ...
    Path getLocalizedUniqueDir() {
      return new Path(baseDir, new Path(subDir, uniqueString));
    }
  }
