...
public class MetricsTimeVaryingRate {
  ...
  /**
   * Push the metrics to the mr, convert to operation per second.
   *  
   * Note this does NOT push to JMX
   * (JMX gets the info via {@link #getPreviousIntervalAverageTime()} and
   * {@link #getPreviousIntervalNumOps()}
   ...
   * @param mr
   */
  public synchronized void pushMetric_OPS(final MetricsRecord mr, int period) {
    intervalHeartBeat();
    try {            
      mr.setMetric(name + "_OPS", getPreviousIntervalNumOps() / period);
      mr.setMetric(name + "_avg_time", getPreviousIntervalAverageTime());
    } catch (Exception e) {
      LOG.info("pushMetric_OPS failed for " + name + "\n" +
          StringUtils.stringifyException(e));
    }
  }
