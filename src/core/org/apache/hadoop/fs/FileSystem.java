...
public abstract class FileSystem extends Configured implements Closeable {
   ...
   * @param f is the path
   *
   * @return a statuses array of the files/directories in the given path
   * @throws IOException If an I/O error occurred
   */
  public LocatedFileStatus[] listLocatedStatus(Path f) throws IOException {
    ArrayList<LocatedFileStatus> results = new ArrayList<LocatedFileStatus>();
    listLocatedStatus(results, f, DEFAULT_FILTER);
   ...
   */
  public void listLocatedStatus(ArrayList<LocatedFileStatus> results, Path f,
      PathFilter filter) throws IOException {
    FileStatus listing[] = listStatus(f, filter);
    if (listing != null) {
      for (int i = 0; i < listing.length; i++) {
        BlockLocation[] locs = listing[i].isDir() ? null
            : getFileBlockLocations(listing[i], 0, listing[i].getLen());
        results.add(new LocatedFileStatus(listing[i], locs));
      }
    }
  }
   ...
   *           if encounter any problem while fetching the status
   */
  public LocatedFileStatus[] listLocatedStatus(Path f, PathFilter filter)
      throws IOException {
    ArrayList<LocatedFileStatus> results = new ArrayList<LocatedFileStatus>();
    listLocatedStatus(results, f, filter);
    return results.toArray(new LocatedFileStatus[results.size()]);
  }
  ...
  static class Cache {
    ...
    static class Key {
      ...
      Key(URI uri, Configuration conf) throws IOException {
        ...
        password = ugi == null? null: ugi.getPassword();
      }
      ...
      public boolean equals(Object obj) {
        ...
        if (obj != null && obj instanceof Key) {
          ...
          return isEqual(this.scheme, that.scheme)
                 ...
                 && isEqual(this.password, that.password);
        }
        return false;        
      }
