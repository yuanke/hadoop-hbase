...
public class UnixUserGroupInformation extends UserGroupInformation {
  ...
  private static char passwdMarker = '#';
  ...
  private void setUserGroupNames(String userName, String[] groupNames) {
    ...
    this.passwd = getPasswordFromStr(groupNames[groupNames.length -1]);
  ...
  private static String getPasswordFromStr(String pass) {
	  if (passwdMarker == pass.charAt(0)) {
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    passwd = getPasswordFromStr(groupNames[numOfGroups -1]);    
  }
  ...
  public static UnixUserGroupInformation readFromConf(
    ...
    if (ugi.length>0 ){
      String userAndPass = ugi[0] + passwdMarker + getPasswordFromStr(ugi[ugi.length - 1]);
      currentUGI = user2UGIMap.get(userAndPass);
    }
    if (currentUGI == null) {
      try {
        currentUGI = new UnixUserGroupInformation(ugi);
        user2UGIMap.put(currentUGI.getUserName() + passwdMarker + currentUGI.getPassword(), currentUGI);
  ...
  public static UnixUserGroupInformation login() throws LoginException {
    try {
      ...
      UnixUserGroupInformation ugi = user2UGIMap.get(userName + passwdMarker);
      if (ugi != null) {
        return ugi;
      }
      ...
      user2UGIMap.put(ugi.getUserName() + passwdMarker + ugi.getPassword(), ugi);
      return ugi;
    } catch (Exception e) {
      throw new LoginException("Login failed: "+e.getMessage());
    }
  ...
  public String toString() {
    ...
    buf.append(userName);    
    int lenGroupNames = groupNames.length;
    if (passwd.length() > 0) {
    	lenGroupNames -= 1;
    }
    for (int i = 0; i < lenGroupNames; i ++) {
      buf.append(',');
      buf.append(groupNames[i]);
    }
    return buf.toString();
  }
}
