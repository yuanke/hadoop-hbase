...
public class DFSUtil {
  ...
  public static final int MAX_DIR_FILE_NAME_LENGTH = 255;
  /**
   * Whether the pathname is valid.  Currently prohibits relative paths, 
   * and names which contain a ":" or "/" 
   */
  public static boolean isValidName(String src) {
    return isValidName(src, false);
  }

  /**
   * Whether the pathname is valid.  Currently prohibits relative paths, 
   * and names which contain a ":" or "/" and names who's length exceed 255
   * @author liangly
   */
  public static boolean isValidName(String src, boolean limitLen) {
    ...
    while(tokens.hasMoreTokens()) {
      ...
      if (element.equals("..") || 
          ...
          (element.indexOf(":") >= 0)  ||
          (element.indexOf("/") >= 0) ||
          (limitLen && element.length() > MAX_DIR_FILE_NAME_LENGTH)) {
        return false;
      }
    }
    return true;
