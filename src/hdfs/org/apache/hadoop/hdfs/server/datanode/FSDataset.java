...
public class FSDataset implements FSConstants, FSDatasetInterface {
  ...
  class FSDir {
    ...
    void getVolumeMap(HashMap<Block, DatanodeBlockInfo> volumeMap, FSVolume volume, boolean repaired) {
      if (children != null) {
        for (int i = 0; i < children.length; i++) {
          children[i].getVolumeMap(volumeMap, volume, repaired);
        }
      }
      ...
      for (int i = 0; i < blockFiles.length; i++) {
        if (Block.isBlockFilename(blockFiles[i])) {
          ...
          volumeMap.put(block, new DatanodeBlockInfo(volume, blockFiles[i]));
          if (repaired)
            DataNode.LOG.warn("Repairing block " + block.getBlockId() + " from vol"
                + volume.dataDir.dir.getAbsolutePath());
        }
      }
    }
  ...
  class FSVolume {
    ...
    void startDuThread() {
      DataNode.LOG.info("start DU thread: " + dataDir.dir.getParent());
      dfsUsage.start();
    }
    
    void stopDuThread() {
      DataNode.LOG.info("stop DU thread: " + dataDir.dir.getParent());
      dfsUsage.shutdown();
    }
    ...
    void getVolumeMap(HashMap<Block, DatanodeBlockInfo> volumeMap, boolean repaired) {
      dataDir.getVolumeMap(volumeMap, this, repaired);
    }
  ...
  static class FSVolumeSet {
    ...
    int curVolume = 0;
    List<FSVolume> crashedVolumes = new ArrayList<FSVolume>();
    ...
    synchronized void getVolumeMap(HashMap<Block, DatanodeBlockInfo> volumeMap) {
      for (int idx = 0; idx < volumes.length; idx++) {
        volumes[idx].getVolumeMap(volumeMap, false);
      }
    }
      
    synchronized List<FSVolume> checkDirs() throws AllDiskErrorException {
      List<FSVolume> badVolumes = new ArrayList<FSVolume>();
      for (int idx = 0; idx < volumes.length; idx++) {
        try {
          volumes[idx].checkDirs();
        } catch (DiskErrorException e) {
          // find bad disk, reset current volume index to 0 to avoid
          // ArrayIndexOutOfBoundsException
          curVolume = 0;
          volumes[idx].stopDuThread();
          synchronized(crashedVolumes){
            crashedVolumes.add(volumes[idx]);
          }
          badVolumes.add(volumes[idx]);
          volumes[idx] = null;
        }
      }
      if (badVolumes.size() > 0) {
        FSVolume newVolumes[] = new FSVolume[volumes.length - badVolumes.size()];
        for (int idx = 0, idy = 0; idx < volumes.length; idx++) {
          if (volumes[idx] != null) {
            newVolumes[idy] = volumes[idx];
            idy++;
          }
        }
        volumes = newVolumes;
      }
      //DataNode.LOG.info("---------- volumes[] size = " + volumes.length + " -----------\n");
      if (volumes.length == 0) {
        throw new AllDiskErrorException("All disk(s) error: ");
      }
      return badVolumes;
    }
    ...
    //added by wukong
    synchronized List<FSVolume> checkCrashedVolumes(){
    	if(crashedVolumes.size() == 0)
        return null;
    	boolean hasRecoveredVolumes = false;
    	List<FSVolume> repairedVolumes = new ArrayList<FSVolume>();
    	Iterator<FSVolume> it = crashedVolumes.iterator();
    	while(it.hasNext()){
    		try{
    			FSVolume vol = it.next();
    			vol.checkDirs();
    			vol.startDuThread();
    			hasRecoveredVolumes = true;
    			repairedVolumes.add(vol);
    			it.remove();
    		}catch(DiskErrorException de){
    		}
    	}
    	//DataNode.LOG.info("---------- volumes[] size before crash check = " + volumes.length + " -----------\n");
    	if(hasRecoveredVolumes){
    		List<FSVolume> goodVolumes = new ArrayList<FSVolume>();
    		for(int i=0; i < volumes.length; i++){
    			goodVolumes.add(volumes[i]);
    		}
    		//goodVolumes.addAll(repairedVolumes);
    		for(int i=0; i < repairedVolumes.size(); i++){
    			goodVolumes.add(repairedVolumes.get(i));
    		}
    		volumes = goodVolumes.toArray(new FSVolume[0]);
    	}
    	//DataNode.LOG.info("---------- volumes[] size after crash check = " + volumes.length + " -----------\n");
    	return repairedVolumes;
    }
  }
  ...
  public FSDataset(DataStorage storage, Configuration conf) throws IOException {
    ...
    List<FSVolume> volList = new ArrayList<FSVolume>();
    for (int idx = 0; idx < storage.getNumStorageDirs(); idx++) {
      try {
        FSVolume newVolume = new FSVolume(storage.getStorageDir(idx)
            .getCurrentDir(), conf);
        volList.add(newVolume);
      } catch (IOException ee) {
        DataNode.LOG.info("Error on newing FSVolume on "
            + storage.getStorageDir(idx).getCurrentDir());
      }
    }
    if (volList.size() == 0)
      throw new IOException("No storage dir is avalible!");
    FSVolume[] volArray = new FSVolume[volList.size()];
    for (int i = 0; i < volList.size(); i++)
      volArray[i] = volList.get(i);
   ...
   * @throws DiskErrorException
   */
  public List<FSVolume> checkDataDir() throws AllDiskErrorException {
    List<FSVolume> badList = volumes.checkDirs();
  ...
  //added by wukong
  public List<FSVolume> checkCrashedVolumes(){
	  return volumes.checkCrashedVolumes();
  }
  
  public void repairVolumeMap(FSVolume vol) {
    vol.getVolumeMap(volumeMap, true);
  }
