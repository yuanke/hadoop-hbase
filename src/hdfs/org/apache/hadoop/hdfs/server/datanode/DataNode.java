...
public class DataNode extends Configured 
  ...
  private static long CRASH_VOLUME_CHECK_INTERVAL = 10; //added by wukong,default for 10 mins
  
  // For InterDataNodeProtocol
  public Server ipcServer;

  private Configuration conf;
  private long lastCheckVolumeTime = 0;
  private long checkVolumeInterval;
  ...
  void startDataNode(Configuration conf, 
    ...
    this.blockReportInterval =
      conf.getLong("dfs.blockreport.intervalMsec", BLOCKREPORT_INTERVAL);//1 hour
    ...
    this.heartBeatInterval = conf.getLong("dfs.heartbeat.interval", HEARTBEAT_INTERVAL) * 1000L;  //3 sec default
    ...
    //added by wukong
    //crash volume check interval, default 10 min
    CRASH_VOLUME_CHECK_INTERVAL = conf.getLong("dfs.crash.recheck.interval", 10); //added by wukong
    CRASH_VOLUME_CHECK_INTERVAL = CRASH_VOLUME_CHECK_INTERVAL * 60 * 1000;

    this.checkVolumeInterval = conf.getLong("dfs.checkVolume.intervalMsec", 3600000);// default 1 hour
  }
  ...
  protected boolean checkDiskError( ) throws IOException {
    boolean blockReported = false;
    try {
      List<FSVolume> badVolumes = data.checkDataDir();
      // more than one volume crash
      if (badVolumes != null && badVolumes.size() > 0) {
        for (FSVolume volume : badVolumes)
          this.unlockStorage(volume);
        restartBlockScanner();
        blockReported = true;
        handleDiskError();
      }
    ...
    return blockReported;
  }
  
  private void stopAllVolumes() {
    LOG.warn("Stopping all operations on volumes");    
    // Unlock all volumes
    for (Iterator<StorageDirectory> it = storage.dirIterator(); it.hasNext();) {
      ...
      try {
        ...
        LOG.warn("Unlock Failed. Vol:" + sd.getCurrentDir().getAbsolutePath());
        e.printStackTrace();
      }
    }    
    // stop block scanner
    LOG.info("Stopping block scanner.");
    blockScannerThread.stop();
    blockScanner.shutdown();
  }
  
  private void handleAllDiskError(String errMsgr) {
  ...
  private void handleDiskError() {
      try {
          ...
          if (resetBlockReportTime) {
            lastBlockReport = brStartTime - R.nextInt((int)(blockReportInterval));
            resetBlockReportTime = false;
          } else {
            lastBlockReport = brStartTime;
          }
          processCommand(cmd);
	} catch (IOException e) {
  ...
  public void offerService() throws Exception {
    ...
    while (shouldRun) {
      try {
        ...
        boolean blockReported = false;
        if (startTime - lastCheckVolumeTime > checkVolumeInterval) {
          blockReported = checkDiskError();
          lastCheckVolumeTime = startTime;
        }
        
        
        
        // send block report
        if (startTime - lastBlockReport > blockReportInterval/* 1 hour */) {
          //
          // Send latest blockinfo report if timer has expired.
          // Get back a list of local block(s) that are obsolete
          // and can be safely GC'ed.
          //
          long brStartTime = now();  //block report start time          
          if (blockReported == false) {
            Block[] bReport = data.getBlockReport();            
  ...
  void unlockStorage(FSVolume vol) {
    for (Iterator<StorageDirectory> it = storage.dirIterator(); it.hasNext();) {
      StorageDirectory sd = it.next();
      if (sd.getCurrentDir().getAbsolutePath()
          .equalsIgnoreCase(vol.getDir().getAbsolutePath())) {
        try {
          sd.unlock();
        } catch (IOException e) {
          LOG.warn("Unlock Failed. Vol:" + vol.getDir().getAbsolutePath());
          e.printStackTrace();
        }
      }
    }
  ...
  void relockStorage(FSVolume vol) {
    for (Iterator<StorageDirectory> it = storage.dirIterator(); it.hasNext();) {
      StorageDirectory sd = it.next();
      if (sd.getCurrentDir().getAbsolutePath()
          .equalsIgnoreCase(vol.getDir().getAbsolutePath())) {
        try {
          ...
          LOG.warn("Lock Failed. Vol:" + vol.getDir().getAbsolutePath());
          e.printStackTrace();
        }
      }
    }
  ...
  void restartBlockScanner() {
    // restart block scanner
    blockScannerThread.stop();
    blockScanner.shutdown();
    LOG.info("Restarting block scanner.");
    blockScanner = new DataBlockScanner(this, (FSDataset)data, this.conf);
    blockScannerThread = new Daemon(blockScanner);
    blockScannerThread.start();
  }
  
  // added by wukong
  // scrashedVolume looping check thread
  class CrashVolumeChecker implements Runnable {

    public void run() {
      while (true) {
        // System.out.println(" === The CrashVolumeChecker thread has started ==="
        // + new Date());
        List<FSVolume> repairedVolumes = data.checkCrashedVolumes();
        if (repairedVolumes != null && repairedVolumes.size() > 0) {
          for (FSVolume vol : repairedVolumes) {
            relockStorage(vol);
            data.repairVolumeMap(vol);
          }
          restartBlockScanner();
          reBlockReport();
          shouldRun = true;
          /*
           * try { data.checkDataDir(); reBlockReport(); } catch
           * (DataNodeDiskErrorException de) { handleDiskError(de.getMessage());
           * } catch (AllDiskErrorException de) {
           * handleAllDiskError(de.getMessage()); }
           */
        }
