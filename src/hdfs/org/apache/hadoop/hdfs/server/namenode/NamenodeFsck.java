...
public class NamenodeFsck {
  ...
  public NamenodeFsck(Configuration conf,
                      ...
                      HttpServletResponse response, InetAddress remoteAddress) throws IOException {
    this.conf = conf;
    this.nn = nn;
  ...
  public void fsck() throws IOException {
    try {
      String msg = "FSCK started by " + UserGroupInformation.getCurrentUGI()
          + " from " + remoteAddress + " for path " + path + " at " + new Date();
      LOG.info(msg);
      out.println(msg);
      nn.namesystem.logFsckEvent(path, remoteAddress);

      FileStatus[] files = nn.namesystem.dir.getListing(path, false);
