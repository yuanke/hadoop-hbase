...
import org.apache.hadoop.security.ExtendAccessControlList;
...
public class FSNamesystem implements FSConstants, FSNamesystemMBean {
  ...
  private ExtendAccessControlList extendAccessControlList;
  ...
  private void startFileInternal(String src,
    ...
    if (!DFSUtil.isValidName(src, true)) {
   ...
   */
  public void invalidateBlock(Block blk, DatanodeInfo dn)
    throws IOException {
  ...
  private boolean renameToInternal(String src, String dst
    ...
    if (!DFSUtil.isValidName(dst, true)) {
  ...
  public FileStatus[] getListing(String src) throws IOException {
    ...
    return dir.getListing(src, false);
  }

  /**
   * Get a listing of all files at 'src' with their block location infos.
   */
  public FileStatus[] getLocatedListing(String src) throws IOException {
    readLock(); // locks first to avoid deadlock
    try {
      if (isPermissionEnabled) {
        if (dir.isDir(src)) {
          checkPathAccess(src, FsAction.READ_EXECUTE);
      ...
      if (auditLog.isInfoEnabled()&& isExternalInvocation()) {
        logAuditEvent(UserGroupInformation.getCurrentUGI(),
                      Server.getRemoteIp(),
                      "listLocatedStatus", src, null, null);
      }
      return dir.getListing(src, true); 
    } finally {
      readUnlock();
    }
  }
  ...
  */
  ...
  /**
   * Get blocks to invalidate for the <i>nodeId</i>
  ...
  public void processReport(DatanodeID nodeID, 
    ...
    if (node.numBlocks() == 0) {
      NameNode.stateChangeLog.info("BLOCK* NameSystem.processReport: "
          + "from " + nodeID.getName() + " " + newReport.getNumberOfBlocks()
          + " blocks" + " shortCircuit first report.");
      Block iblk = new Block(); // a fixed new'ed block to be reused with index i
      for (int i = 0; i < newReport.getNumberOfBlocks(); ++i) {
        iblk.set(newReport.getBlockId(i), newReport.getBlockLen(i), newReport
            .getBlockGenStamp(i));
  ...
  /**
   * @deprecated user {@link NameNode#getNameNodeAddress()} instead.
   */
  @Deprecated
  ...
  private void checkSuperuserPrivilege() throws AccessControlException {
    if (isPermissionEnabled) {
      PermissionChecker pc = new PermissionChecker(
          fsOwner.getUserName(), supergroup, userPasswordInformation, extendAccessControlList);
      if (!pc.isSuper) {
