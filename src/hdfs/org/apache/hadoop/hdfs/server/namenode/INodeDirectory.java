...
public class INodeDirectory extends INode {
  ...
  <T extends INode> INodeDirectory addToParent(byte[][] pathComponents,
      T newNode, INodeDirectory parent, boolean inheritPermission)
      throws FileNotFoundException {
    int pathLen = pathComponents.length;
    if (pathLen < 2) // add root
      return null;
  ...
  long[] computeContentSummary(long[] summary) {
    ...
    if (this instanceof INodeDirectoryWithQuota) {
      ...
      if (-1 != node.getDsQuota() && space != summary[3]) {
        NameNode.LOG.warn("Inconsistent diskspce for directory "
