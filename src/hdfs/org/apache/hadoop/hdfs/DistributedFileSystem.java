...
public class DistributedFileSystem extends FileSystem {
  ...
  public LocatedFileStatus[] listLocatedStatus(Path f) throws IOException {
    LocatedFileStatus[] infos = dfs.listPathWithLocations(getPathName(f));
    if (infos == null) return null;
    for (int i = 0; i < infos.length; i++) {
      infos[i].makeQualified(DistributedFileSystem.this);
    }
    return infos;
  }
  ...
  public void listLocatedStatus(ArrayList<LocatedFileStatus> results, Path f,
    ...
    LocatedFileStatus listing[] = dfs.listPathWithLocations(getPathName(f));
    if (listing != null) {
      for (int i = 0; i < listing.length; i++) {
        if (filter.accept(listing[i].getPath())) {
          listing[i].makeQualified(DistributedFileSystem.this);
