...
<%!
	...
	void counterReset () { colNum = 0; rowNum = 0; reckNum = 0;}
  ...
  String prevReck = "default-reck";
  Pattern p = Pattern.compile("^(r[0-9]{2}[a-z][0-9]{2})[0-9]{3}$");
  String rackTxt(String host, float used) {
    String reck = "default-reck";
    if (p.matcher(host).matches()) {
      reck = host.substring(0, 6);
    }
    if (!reck.equals(prevReck)) {
      prevReck = reck;
      reckNum++;
      }
      
    if (used >= 80.0f) {
      return "<tr class=\"rowHighlight\"> "; 
    }
    return "<tr class=\"" + ((reckNum % 2 == 0)? "rowNormal" : "rowAlt") + "\"> "; 
  }
...
throws IOException {
  ...
  */
  ...
  out.print( rackTxt((( idx > 0 ) ? name.substring(0, idx) : name), d.getDfsUsedPercent())
             + "<td class=\"name\"><a title=\""
             + d.getHost() + ":" + d.getPort() +
             "\" href=\"" + url + "\">" +
...
throws IOException {
  ...
  if (live.isEmpty() && dead.isEmpty()) {
    ...
    if(whatNodes.equals("LIVE")) {
      out.print("<a name=\"LiveNodes\" id=\"title\">" +
