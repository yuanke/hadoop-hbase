...
%>
<html><body>
<h1>Hadoop Job <%=jobid %> on <a href="jobhistory.jsp">History Viewer</a></h1>

<b>User: </b> <%=job.get(Keys.USER) %><br/> 
<b>JobName: </b> <%=job.get(Keys.JOBNAME) %><br/> 
<b>JobConf: </b> <a href="jobconf_history.jsp?jobid=<%=jobid%>&logFile=<%=encodedLogFileName%>&jobLogDir=<%=new Path(logFile).getParent().toString()%>&jobUniqueString=<%=jobUniqueString%>"> 
...
<%
	...
	String counterString = job.get(Keys.COUNTERS);
	Counters totalCounters = Counters.fromEscapedCompactString(counterString);	
	Counters mapCounters = Counters.fromEscapedCompactString(job.get(Keys.MAP_COUNTERS));
    Counters reduceCounters = Counters.fromEscapedCompactString(job.get(Keys.REDUCE_COUNTERS));
    ...
    for (JobHistory.Task task : tasks.values()) {
      ...
      if(Values.MAP.name().equals(task.get(Keys.TASK_TYPE))){
      		Counters tmpMapCounters = Counters.fromEscapedCompactString(task.get(Keys.COUNTERS));
      	}
      	if(Values.REDUCE.name().equals(task.get(Keys.TASK_TYPE))) {
      		Counters tmpReduceCounters = Counters.fromEscapedCompactString(task.get(Keys.COUNTERS));
      	}
