...
public class TestBlockReport extends TestCase {
  ...
  public TestBlockReport(String testName) {
    super(testName);
  }
  ...
  /**
   * Test the case when a block report processing at namenode startup time is
   * fast.
   */
  public void testFasterBlockReports() throws Exception {
    ...
    try {
      ...
      LocatedBlocks locations = namenode.getBlockLocations(file1.toString(), 0,
          Long.MAX_VALUE);
      ...
      LOG.info("Average of all block report processing time " + " from "
          + dinfo.length + " datanodes is " + (total / dinfo.length)
          + " milliseconds.");
    } finally {
      if (cluster != null) {
        cluster.shutdown();
