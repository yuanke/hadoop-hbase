...
public class TestKillSubProcesses extends TestCase {
  ...
  /**
   * Runs a job, kills the job and verifies if the map task and its subprocesses
   * are also killed properly or not.
   */
  static JobID runJobKill(JobTracker jt, JobConf conf) throws IOException {
    ...
    for (TaskReport tr : mapReports) {
      ...
      for (Iterator<TaskAttemptID> it = tip.getActiveTasks().keySet()
        ...
        Path pidFilePath = TaskTracker.getPidFilePath(id, confForThisTask, tip.isCleanupAttempt(id));
        while (pidFilePath == null) {
          ...
          pidFilePath = TaskTracker.getPidFilePath(id, confForThisTask, tip.isCleanupAttempt(id));
        }
    ...
    if (ProcessTree.isSetsidAvailable) {
      String childPid = ProcessTree.getPidFromPidFile(scriptDirName
          + "/childPidFile" + 0);
      while (childPid == null) {
        try {
          Thread.sleep(500);
        ...
        childPid = ProcessTree.getPidFromPidFile(scriptDirName
            + "/childPidFile" + 0);
      }
      ...
      for (int i = 0; i <= numLevelsOfSubProcesses; i++) {
        ...
        assertTrue("Unexpected: The subprocess at level " + i
            + " in the subtree is not alive before killJob", ProcessTree
            .isAlive(childPid));
      }
    }
    ...
    if (ProcessTree.isSetsidAvailable) {
      for (int i = 0; i <= numLevelsOfSubProcesses; i++) {
        ...
        LOG.info("pid of the descendant process at level " + i
            ...
            + " is " + childPid);
        assertTrue("Unexpected: The subprocess at level " + i
            + " in the subtree is alive after killJob", !ProcessTree
            .isAlive(childPid));
      }
    }
    ...
    public void configure(JobConf conf) {
      try {
        ...
        if (ProcessTree.isSetsidAvailable) {
          ...
          String script = "echo $$ > " + scriptDir.toString() + "/childPidFile"
              + "$1\n" + "echo hello\nsleep 1\n" + "if [ $1 != 0 ]\nthen\n"
              + " sh " + shellScript + " $(($1-1))\n" + "else\n"
              + " while true\n do\n" + "  sleep 2\n" + " done\n" + "fi";
