...
public class TestTaskCommit extends HadoopTestCase {
   ...
   * files left bebind in abortTask. We want the test case to 
  ...
  /**
   * Map Task that overrides localizeConfiguration method.
   */
  class TestMapTask extends MapTask {
    public TestMapTask(String jobFile, TaskAttemptID taskId, 
        int partition, String splitClass, BytesWritable split) {
      super(jobFile, taskId, partition, splitClass, split);
    }
    
    @Override
    public void localizeConfiguration(JobConf conf) throws IOException {
      // mock based on Task::localizeConfiguration()
      conf.set("mapred.tip.id", getTaskID().getTaskID().toString()); 
      conf.set("mapred.task.id", getTaskID().toString());
      conf.setBoolean("mapred.task.is.map", isMapTask());
      conf.setInt("mapred.task.partition", getPartition());
      conf.set("mapred.job.id", getTaskID().getJobID().toString());
    }
  ...
  public void testTaskCleanupDoesNotCommit() throws Exception {
    ...
    Task task = new TestMapTask("", attemptID, 0, "", new BytesWritable());
    task.setConf(job);
    task.localizeConfiguration(job);
    task.initialize(job, task.getJobID(), Reporter.NULL, false);
  ...
  public void testCommitRequiredForMapTask() throws Exception {
    ...
    assertTrue("Map task should need commit", 
  ...
  public void testCommitRequiredForReduceTask() throws Exception {
    ...
    assertTrue("Reduce task should need commit", 
        testTask.isCommitRequired());
  }
  ...
  public void testCommitNotRequiredForJobCleanup() throws Exception {
    ...
    assertFalse("Job clenaup task should not need commit", 
        testTask.isCommitRequired());
  }
  ...
  public void testCommitNotRequiredForTaskCleanup() throws Exception {
    ...
    assertFalse("Task clenaup task should not need commit", 
        testTask.isCommitRequired());
  }
  
  private Task createDummyTask(TaskType type) throws IOException,
    ...
    if (type == TaskType.MAP) {
      ...
      taskTask = new MapTask("", attemptId, 0, "", new BytesWritable());
    } else {
      TaskAttemptID attemptId = getTaskAttemptID(false);
      taskTask = new ReduceTask("", attemptId, 0, 1);
    }
    taskTask.setConf(conf);
    taskTask.initialize(conf, taskTask.getJobID(), Reporter.NULL, false);
    return taskTask;
  }
