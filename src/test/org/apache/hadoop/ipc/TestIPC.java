...
public class TestIPC extends TestCase {
  ...
  //from patch HADOOP-6498
  private static class LongErrorWritable extends LongWritable {
    private final static String ERR_MSG = "Come across an exception while reading";
  ...
  public void testErrorClient() throws Exception {
    ...
    try {
      client.call(new LongErrorWritable(RANDOM.nextLong()), addr);
      fail("Expected an exception to have been thrown");
    } catch (IOException e) {
      // check error
      Throwable cause = e.getCause();
